...
class AlphaVantageV4:
    ...
    # --- UPDATED: Added scrub_list parameter ---
    def build_pool(self, num_lineups, exp_limit, scrub_list):
        final_pool, player_counts, indices_store = [], {}, []
        progress_bar = st.progress(0)
        
        # LOGIC: Remove scrubbed players from the data before the loop starts
        filtered_df = self.raw_df[~self.raw_df['Name'].isin(scrub_list)].reset_index(drop=True)
...
if uploaded_file:
    ...
    # --- NEW: Sidebar Scrub Controls ---
    st.sidebar.header("🕹️ Architect Controls")
    all_players = sorted(raw_data['Name'].unique().tolist())
    # This creates the multiselect box in your sidebar
    scrub_list = st.sidebar.multiselect("🚫 Scrub OUT Players (Injuries)", all_players)
    
    exp_limit = st.sidebar.slider("Exposure Cap", 0.1, 1.0, 0.6)
    
    if st.button("🚀 GENERATE LINEUPS"):
        with st.status("Cleaning pool and solving...", expanded=True) as status:
