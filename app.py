...
st.set_page_config(page_title="VANTAGE-V7.3 PRO", layout="wide", page_icon="🛡️")
...
class VantageProV7:
    ...
    def auto_calibrate_settings(self):
        ...
        if num_teams <= 4: return 1.35, 0.80, 0.45, 1 
        return 1.05, 0.75, 0.55, 1
    ...
    def build_pool(self, num_lineups, exp_limit, final_scrubs, leverage_weight, min_sal, min_sharks):
        ...
        for n in range(num_lineups):
            ...
            sim_df['Sim'] = sim_df['Market_Proj'] * sim_df['Team'].map(team_shocks) * np.random.normal(1, 0.12, len(sim_df))
            ...
            # Late-Flex Rule: If a 7:30 player is in the lineup, they MUST occupy UTIL over a 7:00 player
            for i in sim_df.index:
                if sim_df.loc[i, 'Is_Late'] == 0:
                    # Non-late players are forbidden from UTIL if a late player is selected in the lineup
...
if uploaded_file:
    ...
    st.sidebar.header("🕹️ V7.3 Quant Controls")
    ...
    if st.button("🔥 GENERATE V7.3 PORTFOLIO"):
        ...
        engine.df = engine.raw_df.copy() # Essential link
        engine.generate_proprietary_projections(a_alp, {'Donovan Mitchell': 1.22, 'Scottie Barnes': 1.28})
        pool = engine.build_pool(num_lineups, exp_limit, auto_scrubs, leverage_weight, 49750, a_sharks)
        ...
        st.download_button("💾 Download V7.3 CSV", data=pd.DataFrame(csv_rows, columns=['PG', 'SG', 'SF', 'PF', 'C', 'G', 'F', 'UTIL']).to_csv(index=False), file_name="vantage_v7_3.csv")
