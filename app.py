...
# --- VANTAGE 99: INSTITUTIONAL PORTFOLIO ENGINE (V36.0) ---
...
class IndustrialOptimizer:
    ...
    def generate_portfolio(self, n_sims=500, portfolio_size=20, max_exposure=0.5, jitter=0.20):
        ...
        for i in range(n_sims):
            ...
            # EXPOSURE GOVERNOR: Blackout players exceeding cap
            for idx, name in enumerate(self.df['Name']):
                if exposure_counts[name] >= (portfolio_size * max_exposure):
                    # Force player out of this specific simulation
            ...
            if res.success:
                ...
                l_df = self.df.iloc[idx_list]
                
                # Verify uniqueness
                current_names = set(l_df['Name'].tolist())
                if not any(current_names == set(f['Names']) for f in final_lineups):
                    # Assemble and Track
                    latest = l_df['Time'].max()
                    rost = {}
                    p_pool = l_df.copy()
                    for slot, cond in [('C','is_C'),('PG','is_PG'),('SG','is_SG'),('SF','is_SF'),('PF','is_PF'),('G','is_G'),('F','is_F')]:
                        m = p_pool[(p_pool[cond]==1) & (p_pool['Time'] != latest)].sort_values('Proj', ascending=False).head(1)
                        if m.empty: m = p_pool[p_pool[cond]==1].sort_values('Proj', ascending=False).head(1)
                        if not m.empty:
                            rost[slot] = f"{m.iloc[0]['Name']} ({m.iloc[0]['ID']})"
                            p_pool = p_pool.drop(m.index)
                    rost['UTIL'] = f"{p_pool.iloc[0]['Name']} ({p_pool.iloc[0]['ID']})"
                    
                    # Log Exposure
                    for name in current_names: exposure_counts[name] += 1
...
if f:
    ...
    if st.button("🚀 COOK INDUSTRIAL PORTFOLIO"):
        ...
        if portfolio:
            st.success(f"🏆 PORTFOLIO VERIFIED: {len(portfolio)} Optimized Lineups Produced in {round(duration, 2)}s")
            ...
            c3.metric("DIVERSITY GRADE", "A+ (Locked)")
            ...
            with t1:
                ...
                st.dataframe(exp_df.style.background_gradient(cmap='Greens'))
            with t2:
